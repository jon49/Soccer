...
export async function playerGameAllGet(teamId: number, gameId: number, playerIds: number[]) {
  ...
  let playersGame = await getMany<PlayerGame>(
    playerIds.map((x) => getPlayerGameKey(teamId, gameId, x)),
  );
...
function upgradePositions(positions: any) {
  return [positionsToV0].reduce((x, f) => f(x), positions);
}
...
export const statIds: Record<StatName, number> = <any>Object.keys(statDescriptions).reduce(
  (acc, key, i) => {
    acc[key] = i + 1;
    return acc;
  },
...
export async function statsGetAll(teamId: number): Promise<Stats> {
  return (
    (await get<Stats>(getStatsId(teamId))) ?? {
      stats: Object.keys(statDescriptions).map((x, i) => ({
        id: i + 1,
        name: x,
        active: true,
      })),
