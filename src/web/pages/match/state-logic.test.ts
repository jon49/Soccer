...
describe("getTotal", () => {
  ...
  it("ignores intervals without end", () => {
    assert.equal(getTotal([{ start: 100, end: 200 }, { start: 300 }]), 100);
  });
...
describe("getCurrentTotal", () => {
  it("returns completed total when no open interval", () => {
    assert.equal(getCurrentTotal([{ start: 100, end: 200 }]), 100);
  });
...
describe("player status predicates", () => {
  ...
  it("isOnDeckPlayer matches onDeck status", () => {
    assert.equal(
      isOnDeckPlayer(makePlayerGame({ status: { _: "onDeck", targetPosition: 1 } })),
      true,
    );
