...
<button id="dec-${row}" formaction="?teamId=${teamId}&handler=gridSize&action=dec&row=${row}" class=inc>-</button>
      ...
      yield xs.map((x) => html`<input id="position${count++}" name="names[]" value="${x}">`);
      yield html`</fieldset>
...
function getCircles({ count, y, total }: { count: number; y: number; total: number }) {
  ...
  for (let i = 0; i < count; i++) {
    circles += `<circle cx="${x * (i + 1)}%" cy="${cy}%" r="3%" fill="#506f8d" />`;
  }
  return circles;
}
...
<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%">
  <text x="1%" y="10%" fill="#506f8d" font-weight="bold" font-size="20">$${formationName}</text>
  $${function* rows() {
...
const positionNames = ["Striker", "Forward", "Midfield", "Fullback", "Goalkeeper"];
